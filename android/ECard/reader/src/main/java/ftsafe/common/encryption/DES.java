...
public class DES {
    ...
    public static final byte CD_DES_ECB = 0x01;
    public static final byte CD_DES_CBC = 0x02;
    ...
    public byte[] tripleDESEncrypt(byte[] div, byte[] plain, byte[] key, byte mode) throws ErrMessage {
        if (mode == CD_DES_CBC) {
            DES des = new DES();
        } else if (mode == CD_DES_ECB) {
            DES des = new DES();
        } else
            throw new ErrMessage("DES加密模式不正确");
        return null;
    }
